...
function saveAjax (saveurl,pjson,callback) {
	 ...
	 $.ajax({
	 	url: saveurl,//'CPAJsaveProfessionCultivationPlan'
	 	type: 'post',
	 	dataType: 'json',
	 	data: {"jsonstr": pjson},
	 })
	 .done(function(result) {
	 	...
	 	if (result=="success") {
	 		// statement
	 		alert("增加成功！");
	 		//professionplanQuery(getProfessionid());  //刷新专业计划
	 		callback();
...
function getSchoolProfessions () {
	 ...
	 .done(function(result) {
	 	...
	 	if (result==null) {
	 		...
	 		var j=0
	 		for (var i = 0; i < jsr.shoollist.length; i++) {
	 			
	 			str+='<li data-toggle="collapse" data-target="#collapse-school'+i+'" class="bg-li-1">';
	 			str+='<a>'+ jsr.shoollist[i].schoolname+'</a>'
	 			str+=' <ul id="collapse-school'+i+'" class="nav collapse ">';
	 			while(j<jsr.professionlist.length&&jsr.professionlist[j].school.id==jsr.shoollist[i].id){
	 				//console.log("j="+j);
